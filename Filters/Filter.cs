...
namespace PersonalProxy.Filters
{
	...
	public abstract class Filter
	{
		...
		protected string ConfigPath (string filterName)
		{
			return Path.Combine (Path.Combine (Environment.GetFolderPath (Environment.SpecialFolder.ApplicationData), "PersonalProxy"), filterName + ".txt");
		}
	}
	...
	public static class FilterLoader
	{
		...
		{
			try {
				Type t = Type.GetType ("PersonalProxy.Filters." + name);
